# FAQ

## Can I create an account myself?

No. pesan.ai accounts are created by the pesan.ai team after consultation, agreement, and payment.

---

## Is the consultation free?

Yes. The consultation request is used to understand your business needs before setup.

---

## Why do I need to connect WhatsApp Business?

pesan.ai needs a connected WhatsApp Business Account to access WhatsApp communication features and support the dashboard workflow.

---

## Can I keep using my WhatsApp Business App?

If eligible, the coexistence flow allows an existing WhatsApp Business App account to connect to the WhatsApp Business Platform while still using the app.

---

## Should I share chat history during setup?

No. Choose **Don't share chats** because chat history sync is not currently supported by pesan.ai.

---

## What if I cannot sign in?

Contact the pesan.ai team and confirm that your account has been created and activated.

---

## What if WABA setup fails?

Restart the setup from WABA Management and make sure you use the correct business portfolio, WhatsApp number, and phone access.

---

## What if I selected the wrong business portfolio?

Restart onboarding from WABA Management or contact the pesan.ai team for help.

---

## Can I export customer contacts?

Yes. Open the Customers page and click **Export Excel** to download the customer contact list.
