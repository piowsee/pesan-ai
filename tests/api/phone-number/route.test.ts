import { GET, POST } from '@/app/api/phone-number/route';
...
describe('GET /api/phone-number', { tags: ['backend'] }, () => {
  it('returns unpaginated customer phone numbers with filters', async () => {
    ...
    const req = new Request(
      'http://localhost/api/phone-number?wabaId=waba-1&phoneNumber=%2B6281234567890',
    );
  ...
  it('returns paginated customer phone numbers when pagination is requested', async () => {
    ...
    const req = new Request(
      'http://localhost/api/phone-number?wabaId=waba-1&page=2&limit=5',
    );
    const response = await GET(req, { params: Promise.resolve({}) } as never);
    const data = await response.json();
  ...
  it('returns 500 when unauthorized', async () => {
    ...
    const req = new Request('http://localhost/api/phone-number');
    const response = await GET(req, { params: Promise.resolve({}) } as never);
